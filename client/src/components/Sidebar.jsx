  ...
  return (
    <div className="w-64 min-h-screen border-r bg-white flex flex-col">
      ...
      <div className="p-6 border-b border-gray-50">
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{greet}</p>
        <h2 className="text-2xl font-black text-slate-900 mt-1">AdminPanel</h2>
      </div>
      ...
      <nav className="flex-1 p-4 overflow-y-auto">
        {navSections.map((section, idx) => (
          <div key={idx} className="mb-4">
            ...
            >
              {section.title}
              <span className="text-sm">{openSection === section.title ? '−' : '+'}</span>
            </button>
      ...
      <div className="p-4 border-t border-gray-100">
        <p className="text-xs text-gray-400 text-center">v1.0.4 Build</p>
      </div>
    </div>
  )
}
